Legacy short path implementation can be turned on with the -legacy-short-path option


First, let's setup and example where the legacy and new short-path disagree (of
course, the new one is better!)
...
This is the result with the new implementation:
...
This is the result with the legacy implementation. It does less good with
Base__On_error instead of On_error:

  $ cat >.merlin <<'EOF'
  > FLG -short-paths -open Base__ -legacy-short-path
  > EOF
  ...
  $ $MERLIN single type-enclosing -index 0 -position 1:11 \
  > -filename test.ml < test.ml | jq '.value[0].type'
  "(module Base__Or_error)"

  $ $MERLIN single type-enclosing -index 0 \
  > -log-file log  -position 1:22 \
  > -filename test_f.ml < test_f.ml | jq '.value[0].type'
  "(module Base__Or_error.Id)"
