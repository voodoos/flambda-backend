...
let pp_substs fmt substs =
  ...
  let pp_v fmt (p, s) =
    Format.fprintf fmt "%a -> [%a]" (Format_doc.compat Path.print) p
  ...
  let paths, substs =
    ...
    | Module ->
      ...
      begin
        ...
        | Mty_signature s ->
          let ldot id =
            if for_open then lid else Longident.Ldot
              (Location.mknoloc lid, Location.mknoloc (Ident.name id))
          in
          ...
          List.fold_left
            (fun (paths, substs) ->
              ...
              | Subst.Lazy.Sig_jkind _ -> (paths, substs))
            (paths, substs)
...
let add_used env kind lid path =
  let rec loop acc kind lid path =
    let lid, _loc = (lid.Location.txt, lid.Location.loc) in
    let acc =
    ...
    | Pdot (path, _), Ldot (lid, _) -> loop acc Module lid path
    | Papply (p1, p2), Lapply (l1, l2) ->
      let acc = loop acc Module l1 p1 in
      loop acc Module l2 p2
    | _, _ -> acc
  in
...
and define_component ?root_path ?root_lid sig_item =
  if record_usages then
    ...
    | Types.Sig_typext (_, _, _, _) | Types.Sig_jkind _-> ()
...
let define_signature_for_open _env ~root_path (sg : Subst.Lazy.signature) =
  List.iter
    (fun sig_item ->
      ...
      | Subst.Lazy.Sig_typext (_, _, _, _) | Subst.Lazy.Sig_jkind _-> ()
...
let use_constructor env ( lid)
    (constr : Data_types.constructor_description) =
  if record_usages then begin
    let () =
...
let use_label env ( lid)
    (label : _ Data_types.gen_label_description) =
  if record_usages then begin
    let () =
